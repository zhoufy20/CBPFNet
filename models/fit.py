...
class Fit(object):
    def __init__(self, **train_config):
        ...
        wandb.init(project="cbpfnet", config=self.train_config)
        ...
        if torch.cuda.is_available() and self.device == 'cpu':
            ...
            print('User warning: `CUDA` device is not available, but you choosed `cuda:0`. Change the device to `cpu`.', file=self.log)
            self.device = 'cpu'
        ...
        train_dataset, val_dataset, test_dataset = random_split(self._dataset, [self._train_size, self._val_size, self._test_size])

        # check batch size
        self.train_config['batch_size'] = min(self.train_config['batch_size'], self._train_size)
        self.train_config['val_batch_size'] = min(self.train_config['val_batch_size'], self._val_size, self._test_size)
    ...
    def fit(self, **train_config):
        ...
        for epoch in range(self.train_config['epochs']):
            ...
            epoch_force_loss_train, epoch_energy_loss_train, epoch_stress_loss_train = [], [], []
            ...
            for i, (graph, props) in enumerate(self.train_loader):
                ...
                if self._has_adsorbate:
                    force_true *= torch.reshape(graph.ndata['adsorbate']*self.train_config['adsorbate_coeff']+1., (-1,1))
                    force_pred *= torch.reshape(graph.ndata['adsorbate']*self.train_config['adsorbate_coeff']+1., (-1,1))
                ...
                epoch_force_loss_train.append(force_loss.item())
                epoch_energy_loss_train.append(energy_loss.item())
                epoch_stress_loss_train.append(stress_loss.item())

            # wandb log
            wandb.log({
                'epoch': epoch,
                'train_energy_loss': sum(epoch_force_loss_train),
                'train_force_loss': sum(epoch_energy_loss_train),
                'train_total_loss': sum(epoch_stress_loss_train),
            })
