...
class Score(object):
    ...
    def echo(self, **test_config):
        ...
        with torch.no_grad():
            ...
            contcarpath = "/mnt/e/ComPhys/GitDemo/cbfnet/enpaper/Dataset/CCData/C2H5CCC2H5/C2H5CCC2H5_1/0/CONTCAR"
            fig_path = "/mnt/e/ComPhys/GitDemo/cbfnet/enpaper/CBPFNet/models/output"
            ...
            for i, edge_id in enumerate(edge_ids[1]):
                ...
                if atomconsider == edge_id:
                    ...
                    columnlist.append(i)
            atomneighbor = [atom.numpy() for atom in atomneighbor]
            atomneighbor = np.stack(atomneighbor)

            # calculate the entropy
            # echo the neighbor atom
            for atthead in range(6):
                attentionconsider = [attention[column, atthead, 0] for column in columnlist]
                print(attentionconsider, len(attentionconsider))
                print(attentionconsider)
            ...
            edge_colors = color_map((attentionhead - attentionhead.min()) /
                                    (attentionhead.max() - attentionhead.min()))
