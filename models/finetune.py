# @Time    : 2024/12/26 15:55
# @Author  : Feiyu
# @File    : Model_Test.py
# @diligent：What doesn't kill me makes me stronger.
# @Function: finetune the trained model, .
...
class Finetune(object):
    def __init__(self, **train_config):
        ...
        self.log = open('finetune.log', 'w', buffering=1)
    ...
    def finetune(self, **train_config):
        ...
        # transfer learning.
        # Finetune
        if self.train_config.get('freeze_layers', False):
            for param in model.parameters():
                param.requires_grad = False
            # only finetune the tail_readout
            if hasattr(model, 'tail_readout'): 
                for param in model.tail_readout.parameters():
                    param.requires_grad = True
            else:
                print("Warning: 'tail_readout' not found in the model")
